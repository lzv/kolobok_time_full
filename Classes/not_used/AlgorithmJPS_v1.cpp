#include "AlgorithmJPS_v1.h"
#include "BinaryHeap.h"

// Изначально, как и в алгоритме A*, я сохранял в нодах точек информацию о направлении на родительскую точку, 
// по которому и восстанавливался путь от конца к началу, когда поиск доходил до конечной точки. 
// Однако, в алгоритме Jump Point Search это оказалось ошибкой. Между двумя точками одного пути могла встретиться 
// точка другого пути, которая мало того, что уведет на более длинный путь, так еще может организоваться цикл, 
// когда две ноды (из разных путей) указывают друг на друга. 
// Отладка заняла много времени, но в итоге причина была найдена. Выход - хранить в ноде координаты родительской точки. 
// Тогда, при восстановлении пути от конца к началу, ноды из других путей будут игнорироваться. 
// Так же можно вместо координат хранить указатель на родительскую ноду. Может будет немножко быстрее. 

// Необходимо отметить неоптимальность в поиске прыжковых точек: при движении по диагонали точка считается 
// прыжковой, если от нее по вертикали или горизонтали находится прижковая точка. Приходится искать эту вторую 
// прыжковую точку, а затем, когда начнется обработка найденной диагональной прыжковой точки, искать эту прыжковую 
// точку (по вертикали или горизонтали) второй раз. 
// Можно ускорить алгоритм, если в диагональной прыжковой точке сохранять направление и координаты второй прыжковой 
// точки. 
...
JPSNode * JPSFindPoint (
    ...
    while (
        ...
        && !bit_map[current_x][current_y]
    ) {
        if (JPSIsJumpPoint(current_x, current_y, direction, bit_map, bit_map_width, bit_map_height, end_x, end_y))
        {
        ...
        if (is_diagonal)
        {
            ...
            while (
                ...
                if (JPSIsJumpPoint(check_x, check_y, check_direction, bit_map, bit_map_width, bit_map_height, end_x, end_y))
                {
                    int cost = point->from_begin_cost + JPSDirectionCost[direction] * steps_count;
                    return new JPSNode {current_x, current_y, cost, true, direction, point->x, point->y};
                }
...
bool JPSCheckNeighbors (
    ...
    JPSNode * point = JPSFindPoint(node, node->direction_from_parent, bit_map, bit_map_width, bit_map_height, end_x, end_y);
    ...
    if (
        ...
        point = JPSFindPoint(node, JPSDirectionChecks1[node->direction_from_parent].neighbor, 
                                bit_map, bit_map_width, bit_map_height, end_x, end_y);
        if (point != nullptr) JPSAddPointInOpenList(point, open_heap, p_nodes_array, bit_map_width, end_x, end_y);
    }
    ...
    if (JPSDirectionIsDiagonal[node->direction_from_parent])
    {
        ...
        point = JPSFindPoint(node, add_directions.first, bit_map, bit_map_width, bit_map_height, end_x, end_y);
        if (point != nullptr) JPSAddPointInOpenList(point, open_heap, p_nodes_array, bit_map_width, end_x, end_y);
        
        point = JPSFindPoint(node, add_directions.second, bit_map, bit_map_width, bit_map_height, end_x, end_y);
        if (point != nullptr) JPSAddPointInOpenList(point, open_heap, p_nodes_array, bit_map_width, end_x, end_y);
    }
...
JPSFindResults JPSFindPatch (
    ...
    for (int i = 0; i < pixels_count; ++i) p_nodes_array[i] = nullptr;
    ...
    {
        auto node = JPSFindPoint(start_node, i, bit_map, width, height, end_x, end_y);
        if (node != nullptr) JPSAddPointInOpenList(node, open_heap, p_nodes_array, width, end_x, end_y);
    }
    
    while (!open_heap.empty() && !find_patch)
        find_patch = JPSCheckNeighbors(open_heap, p_nodes_array, bit_map, width, height, end_x, end_y);
